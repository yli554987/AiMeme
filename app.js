...
function renderSticker(targetCanvas, variantStyle) {
  ...
  drawSceneTag(tctx, getSceneLine(), variantStyle);
...
function buildPrompt(styleId = getSelectedStyle()) {
  ...
  return [
    ...
    styleText,
    `识别人物：「${character}」。识别场景：「${scene}」。`,
    "先从视频真人截图中识别人物、动作、字幕台词、可能出现的品牌或商品；主文案也要基于截图情绪自动提炼。",
    `画面情绪和人物动作参考场景台词：「${action}」。这句只作为小气泡或表情动作依据，不要抢主标题。`,
    `画面中必须准确出现主文案：「${caption}」。这是贴纸最大、最醒目的文字。`,
    `不要生成与主文案无关的大字，不要把广告物品名称改写成主标题，不要额外编造口号。装饰文字只能重复或拆分主文案「${caption}」。`,
    adLine,
    "如果参考图 URL 存在，优先把它作为人物/画面视觉参考；视频真人截图用于识别人、动作、台词和整体场景。",
    "构图要求：人物是主体，大头占画面中心；有厚白描边、干净阴影、可直接用于聊天贴纸；广告元素自然融入画面，不像硬广横幅。",
    "文字要求：主文案放在底部或人物旁边的大标题区；场景台词只作为小气泡；字体简单粗大，最多一到两行。",
